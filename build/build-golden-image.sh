#!/usr/bin/env bash
# minivps-db-appliance ゴールデンイメージ ビルドスクリプト。
#
# ベースのUbuntu cloud imageに mysql-server + nftables + 本リポジトリのゲスト内設定
...
# specs/db-1.yaml の base_image をこの名前に書き換えて使う。
#
# 注意: アプリ用DBユーザーは焼き込まない。イメージやgit履歴に秘密が残るのを防ぐため。
# 作成手順はREADME「秘密情報の初期化」を参照。
set -euo pipefail

BASE_IMAGE_NAME="${BASE_IMAGE_NAME:-ubuntu-26.04.img}"
GOLDEN_IMAGE_NAME="${GOLDEN_IMAGE_NAME:-minivps-db-golden-$(date +%Y%m%d).qcow2}"
...
# volumeが残ることを防ぐため。
TEMP_DISK_VOL="minivps-db-build-disk.qcow2"
TEMP_SEED_VOL="minivps-db-build-seed.iso"

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORKDIR="$(mktemp -d /var/tmp/minivps-db-build.XXXXXX)"
...
instance-id: iid-minivps-db-golden-build-001
local-hostname: minivps-db-build
EOF

# mysqlの設定は defer: true でパッケージ導入後・runcmd前に書く。
# cloud-initのwrite_filesは既定でパッケージ導入前に走るため、先に書くと
# /etc/mysql が未作成の状態でファイルだけが置かれ、mysql-server導入時にaptが
# ディレクトリ衝突で失敗する。nftables.confは従来どおり導入前に書く。
cat > "$WORKDIR/user-data" <<EOF
#cloud-config
hostname: minivps-db-build
users:
  - name: ubuntu
...
write_files:
  ...
  - path: /root/golden-finalize.sh
    ...
    content: |
      #!/bin/bash
      set -euxo pipefail
      systemctl enable nftables.service mysql.service
      # サーバUUIDを消す。残したままだとこのイメージから作った全VMが同じUUIDを
      # 持つ。次回起動時にmysqldが再生成する。
